                    ...
                    <table id="pTable" class="table table-responsive table-striped">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ date('d-m-Y', strtotime(substr( $row->created_at, 0, 11))) }}</td>
                                <td>{{ $row->level_from->job_title }}</td>
                                <td>{{ $row->level_to->job_title }}</td>
                                <td>
                                    <a class="btn btn-primary" href="{{ URL('/hrd/pPersonalbod/'.$row->sub_emp_number.'/'.$row->id) }}">
                                        <i>
                                            Data Personal
                                        </i>
                                    </a>
                                        <!-- <a href="#apvModal" data-toggle="modal" class="open-apvModal" data-id="{{ $row->id }}" id="apv"><i class="fa fa-check-circle" title="Approve"></i></a> -->
                                </td>
                            </tr>
                            @endforeach
                            @else
...
<div id="apvModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="apvModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('hrd.setPromotion') }}" method="POST">
                <div class="modal-body">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}" />
                    <input type="hidden" name="id" id="pro_id" value="" />
                    <label for="mt_doc_no">Are you sure approve this request?</label>
